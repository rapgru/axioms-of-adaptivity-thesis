...
Suppose that $`\mathcal{T} : ℕ → \mathbb{T}` is the sequence of meshes generated by
...
$$`∑_{k=l}^{l+n-1} \left(\mathbb{d}[\mathcal{T}_{k+1}; U(\mathcal{T}_{k+1}), U(\mathcal{T}_k)]^2 - ε_{qo} \mathbb{d}[\mathcal{T}_k; u, U(\mathcal{T}_k)]^2\right) ≤ C_{qo} η(\mathcal{T}_l; U(\mathcal{T}_l))^2`.
